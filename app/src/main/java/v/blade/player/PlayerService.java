...
public class PlayerService extends Service {
    ...
    private MediaSessionCompat.Callback mSessionCallback = new MediaSessionCompat.Callback() {
        ...
        public void onPlay() {
            if (mPlayer.getCurrentState() == PlayerMediaPlayer.PLAYER_STATE_DO_NOTHING) {
                ...
                mPlayer.playSong(shuffleMode ? shufflePlaylist.get(currentPosition) :
                        currentPlaylist.get(currentPosition));
        ...
        public void onSetShuffleMode(int shuffleMode) {
            if (!PlayerService.this.shuffleMode) // shuffle mode enabling, construct shuffle playlist
            {
                ...
                currentPosition = 0;
            } else // shuffle mode disable, retrieve current position
            {
                int i;
            ...
            PlayerMediaPlayer.listener.onStateChange();
        }
    };
    ...
    private final PlayerMediaPlayer.MediaPlayerListener mPlayerListener = new PlayerMediaPlayer.MediaPlayerListener() {
        ...
        public void onStateChange() {
            ...
            //build and send current media information
            ...
            mSession.setShuffleMode(shuffleMode ? PlaybackStateCompat.SHUFFLE_MODE_NONE :
            ...
            if (mNotification == null) {
                mNotification = mNotificationManager.getNotification(
                        currentSong, mPlayer.getCurrentState(), mSession.getSessionToken());
                startForeground(PlayerNotification.NOTIFICATION_ID, mNotification);
            } else {
                stopForeground(false);
                mNotification = mNotificationManager.getNotification(
                        currentSong, mPlayer.getCurrentState(), mSession.getSessionToken());
                mNotificationManager.getNotificationManager().notify(
                        PlayerNotification.NOTIFICATION_ID, mNotification);
            }
        }
    };
    ...
    public void setCurrentPlaylist(ArrayList<Song> playlist, int position) {
        ...
        if (shuffleMode) shuffleMode = false;
    }
    ...
    public ArrayList<Song> getCurrentPlaylist() {
        return shuffleMode ? shufflePlaylist : currentPlaylist;
    }
    ...
    public Song getCurrentSong() {
        return currentPlaylist == null ? null : shuffleMode ? shufflePlaylist.get(currentPosition) :
                currentPlaylist.get(currentPosition);
    }
    ...
    public int resolveCurrentSongDuration() {
        return mPlayer.getDuration();
    }

    public int resolveCurrentSongPosition() {
