...
public abstract class Source {
    ...
    public static Source SOURCE_LOCAL_LIB = new Source(R.drawable.ic_local, 0, "Local") {
        ...
        private SourcePlayer player = new SourcePlayer() {
            ...
            public void setListener(PlayerListener listener) {
                ...
                mediaPlayer.setOnCompletionListener(mp -> listener.onSongCompletion());
            }
            ...
            public void play(PlayerCallback callback) {
                ...
                mediaPlayer.start();
                if (callback != null) callback.onSuccess(player);
            }
            ...
            public void playSong(Song song, PlayerCallback callback) {
                ...
                if (song.getFormat() != null && song.getFormat().equals("audio/x-ms-wma")) {
                    Toast.makeText(LibraryService.appContext,
                            LibraryService.appContext.getString(R.string.format_unsupported),
        ...
        public void registerSongs() {
            ...
            if (musicCursor != null && musicCursor.moveToFirst()) {
                ...
                do {
                    ...
                    if (loadAlbumArts)
                        if (idsorted_albums.get(albumId) == null)
                            idsorted_albums.put(albumId, s.getAlbum());
                }
        ...
        public void addSongsToPlaylist(List<Song> songs, Playlist list, OperationCallback callback) {
            if (list.getSources().getSourceByPriority(0).getSource() != SOURCE_LOCAL_LIB) {
                callback.onFailure();
                return;
            }
        ...
        @Override
        public boolean searchForSong(Song song) {
            return false;
        }
    ...
    private Source(int iconImage, int logoImage, String name) {
    ...
    public interface OperationCallback {
        void onSuccess(LibraryObject result);
    ...
    public static class Spotify extends Source {
            ...
            public void init() {
                ...
                com.spotify.sdk.android.player.Spotify.getPlayer(playerConfig, LibraryService.appContext, new SpotifyPlayer.InitializationObserver() {
                    ...
                    public void onInitialized(final SpotifyPlayer p) {
                        ...
                        spotifyPlayer.addConnectionStateCallback(new ConnectionStateCallback() {
                            ...
                            public void onLoginFailed(com.spotify.sdk.android.player.Error error) {
                                Toast.makeText(LibraryService.appContext,
                                        LibraryService.appContext.getString(
                                                R.string.player_login_error) + " Spotify",
                                        Toast.LENGTH_SHORT).show();
                            }
                    ...
                    public void onError(Throwable throwable) {
                        Toast.makeText(LibraryService.appContext,
                                LibraryService.appContext.getString(R.string.player_unknown_error) + " Spotify", Toast.LENGTH_SHORT).show();
                    }
                });
            }
        ...
        @Override
        public String getUserName() {
            return mePrivate == null ? "" : (mePrivate.display_name == null ? mePrivate.id : mePrivate.display_name);
        }
        ...
        public void disconnect() {
            ...
            SharedPreferences accountsPrefs = LibraryService.appContext.getSharedPreferences(
            ...
            try {
                ...
                while (true) {
                    ...
                    for (PlaylistSimple playlistBase : userPlaylists.items) {
                        ...
                        while (trackNbr > 0) {
                            ...
                            for (PlaylistTrack pt : tracks.items) {
                                ...
                                if (LibraryService.SAVE_PLAYLISTS_TO_LIBRARY)
                                    ...
                                    s = LibraryService.getSongHandle(
                                            t.name,
                                            t.album.name,
                                            t.artists.get(0).name,
                                            t.duration_ms,
                                            new SongSources.SongSource(t.id, SOURCE_SPOTIFY),
                                            t.track_number);
                        ...
                        if (playlistBase.owner != null)
                            if (!playlistBase.owner.id.equals(mePrivate.id))
                                list.setOwner(playlistBase.owner.display_name == null ? playlistBase.owner.id : playlistBase.owner.display_name, playlistBase.owner.id);
                ...
                if (isAvailable()) {
                    ...
                    for (kaaes.spotify.webapi.android.models.AlbumSimple a : albums.albums.items) {
                        ...
                        for (Track t : albumTracks.items) {
                            Song currentSong = LibraryService.getSongHandle(t.name, a.name, t.artists.get(0).name, t.duration_ms, new SongSources.SongSource(t.id, SOURCE_SPOTIFY), t.track_number);
        ...
        @Override
        public void addSongsToPlaylist(List<Song> songs, Playlist list, OperationCallback callback) {
            if (list.getSources().getSourceByPriority(0).getSource() != SOURCE_SPOTIFY) {
                callback.onFailure();
                return;
            }
            if (!list.isMine() && !list.isCollaborative()) {
                callback.onFailure();
                return;
            }
            ...
            new Thread() {
                public void run() {
                    ...
                    StringBuilder sSongs = new StringBuilder();
                    for (Song s : songs) {
                        ...
                        sSongs.append("spotify:track:").append(spot.getId()).append(",");
                    }
                    if (sSongs.length() == 0) {
                        callback.onFailure();
                        return;
                    }
                    sSongs = new StringBuilder(sSongs.substring(0, sSongs.length() - 1));
                    parameters.put("uris", sSongs.toString());
        ...
        public boolean searchForSong(Song s) {
            ...
            try {
                ...
                args.put("limit", 1);
                List<Track> t = Source.SOURCE_SPOTIFY.spotifyApi.getService().searchTracks(
                        s.getTitle() + " album:" +
                                s.getAlbum().getName() + " artist:" + s.getArtist().getName(), args).tracks.items;
        ...
        private void cachePlaylist(Playlist p) {
            try {
                ...
                pwriter.write((String) p.getSources().getSpotify().getId());
                pwriter.newLine();
                pwriter.write(String.valueOf(p.isMine()));
                pwriter.newLine();
                if (!p.isMine()) {
                    pwriter.write(p.getOwner());
                    pwriter.newLine();
                    pwriter.write((String) p.getOwnerID());
                    pwriter.newLine();
                }
                pwriter.write(String.valueOf(p.isCollaborative()));
                pwriter.newLine();
                for (Song song : p.getContent()) {
                    pwriter.write(song.getTitle() + CACHE_SEPARATOR +
                            ...
                            song.getSources().getSpotify().getId()
                            + CACHE_SEPARATOR);
                    pwriter.newLine();
                }
                pwriter.close();
            ...
            new Thread() {
                public void run() {
                    try {
                        ...
                        //add to library
                        if (song.isHandled()) {
                            //todo : find a better way (registersong is heavy); that is just laziness
    ...
    public static class Deezer extends Source {
        ...
        public SourcePlayer player = new SourcePlayer() {
            ...
            public void init() {
                try {
                    deezerPlayer = new TrackPlayer((Application)
                            LibraryService.appContext.getApplicationContext(),
                            deezerApi, new WifiAndMobileNetworkStateChecker());
                    setListener(PlayerMediaPlayer.playerListener);
                } catch (Exception e) {
            ...
            @Override
            public void setListener(PlayerListener listener) {
                deezerPlayer.addOnPlayerStateChangeListener((playerState, l) -> {
            ...
            public void play(PlayerCallback callback) {
                ...
                if (deezerPlayer.getPlayerState() == PlayerState.RELEASED) {
                    if (callback != null) callback.onFailure(player);
                    return;
                }
            ...
            try {
                if (deezerCacheFile.exists()) {
                    ...
                    for (File f : deezerPlaylistsCache.listFiles()) {
                        ...
                        if (!isMine) {
                            owner = sppr.readLine();
                            ownerID = Long.parseLong(sppr.readLine());
                        }
                        boolean isCollab = Boolean.parseBoolean(sppr.readLine());
                        while (sppr.ready()) {
                            String[] tp = sppr.readLine().split(CACHE_SEPARATOR);
        ...
        public void registerSongs() {
            ...
            try {
                List<com.deezer.sdk.model.Track> tracks =
                        (List<com.deezer.sdk.model.Track>) JsonUtils.deserializeJson(deezerApi.requestSync(requestTracks));
                ...
                List<com.deezer.sdk.model.Album> albums = (List<com.deezer.sdk.model.Album>)
                        JsonUtils.deserializeJson(deezerApi.requestSync(requestAlbums));
                for (com.deezer.sdk.model.Album album : albums) {
                    Album alb = null;
                    List<com.deezer.sdk.model.Track> albTracks = (List<com.deezer.sdk.model.Track>)
                            JsonUtils.deserializeJson(deezerApi.requestSync(DeezerRequestFactory.requestAlbumTracks(album.getId())));
                ...
                List<com.deezer.sdk.model.Artist> artists = (List<com.deezer.sdk.model.Artist>)
                        JsonUtils.deserializeJson(deezerApi.requestSync(requestArtists));
                for (com.deezer.sdk.model.Artist artist : artists) {
                    List<com.deezer.sdk.model.Album> artistAlbums = (List<com.deezer.sdk.model.Album>)
                            JsonUtils.deserializeJson(deezerApi.requestSync(DeezerRequestFactory.requestArtistAlbums(artist.getId())));
                ...
                for (int i = 0; i < playlists.size(); i++) {
                    ...
                    if (!playlist.getCreator().equals(me))
                        list.setOwner(playlist.getCreator().getName(), playlist.getCreator().getId());
                ...
                try {
                    ...
                    for (Song song : deezerSongs) {
                        bw.write(song.getTitle() + CACHE_SEPARATOR +
                                song.getAlbum().getName() + CACHE_SEPARATOR + song.getArtist().getName() + CACHE_SEPARATOR +
                                song.getFormat() + CACHE_SEPARATOR +
                                song.getTrackNumber() + CACHE_SEPARATOR + song.getDuration() + CACHE_SEPARATOR +
                                song.getSources().getDeezer().getId()
                                + CACHE_SEPARATOR);
                        bw.newLine();
                    }
                    bw.close();
        ...
        public List<LibraryObject> query(String query) {
            ...
            if (isAvailable()) {
                try {
                    //request from deezer
                    List<com.deezer.sdk.model.Track> tracks = (List<com.deezer.sdk.model.Track>)
                            JsonUtils.deserializeJson(deezerApi.requestSync(DeezerRequestFactory.requestSearchTracks(query)));
                    List<com.deezer.sdk.model.Album> albums = (List<com.deezer.sdk.model.Album>)
                            JsonUtils.deserializeJson(deezerApi.requestSync(DeezerRequestFactory.requestSearchAlbums(query)));
                    ...
                    //handle returned data
                    for (com.deezer.sdk.model.Track t : tracks) {
                        Song currentSong = LibraryService.getSongHandle(
                                t.getTitle(),
                                t.getAlbum().getTitle(),
                                t.getArtist().getName(),
                                t.getDuration() * 1000,
                                new SongSources.SongSource(t.getId(), SOURCE_DEEZER),
                                t.getTrackPosition());
                        tr.add(currentSong);
                    ...
                    for (com.deezer.sdk.model.Album alb : albums) {
                        Album album = null;
                        List<com.deezer.sdk.model.Track> albTracks = (List<com.deezer.sdk.model.Track>)
                                JsonUtils.deserializeJson(deezerApi.requestSync(DeezerRequestFactory.requestAlbumTracks(alb.getId())));
                        for (com.deezer.sdk.model.Track t : albTracks) {
                            Song currentSong = LibraryService.getSongHandle(
                                    t.getTitle(),
                                    t.getAlbum().getTitle(),
        ...
        @Override
        public void addSongsToPlaylist(List<Song> songs, Playlist list, OperationCallback callback) {
            if (list.getSources().getSourceByPriority(0).getSource() != SOURCE_DEEZER) {
                callback.onFailure();
                return;
            }
            if (!list.isMine() && !list.isCollaborative()) {
            ...
            new Thread() {
                public void run() {
                    try {
                        ...
                        DeezerRequest request = DeezerRequestFactory.requestPlaylistAddTracks((long)
                                list.getSources().getDeezer().getId(), ids);
        ...
        public void removeSongFromPlaylist(Song song, Playlist list, OperationCallback callback) {
            ...
            DeezerRequest request = DeezerRequestFactory.requestPlaylistRemoveTracks((long)
                    list.getSources().getDeezer().getId(), ids);
        ...
        public boolean searchForSong(Song s) {
            ...
            DeezerRequest search = DeezerRequestFactory.requestSearchTracks("track:\"" +
                    s.getTitle() + "\" album:\"" +
                    s.getAlbum().getName() + "\" artist:\"" +
                    s.getArtist().getName() + "\"");
        ...
        private void cachePlaylist(Playlist p) {
            try {
                ...
                thisPlaylist.createNewFile();
                BufferedWriter pwriter = new BufferedWriter(new FileWriter(thisPlaylist));
                pwriter.write(String.valueOf((long) p.getSources().getDeezer().getId()));
                pwriter.newLine();
                pwriter.write(String.valueOf(p.isMine()));
                pwriter.newLine();
                if (!p.isMine()) {
                    pwriter.write(p.getOwner());
                    pwriter.newLine();
                    pwriter.write(String.valueOf((long) p.getOwnerID()));
                    pwriter.newLine();
                }
                pwriter.write(String.valueOf(p.isCollaborative()));
                pwriter.newLine();
                for (Song song : p.getContent()) {
                    pwriter.write(song.getTitle() + CACHE_SEPARATOR +
                            ...
                            song.getTrackNumber() + CACHE_SEPARATOR +
                            song.getDuration() + CACHE_SEPARATOR +
                            song.getSources().getDeezer().getId() + CACHE_SEPARATOR);
                    pwriter.newLine();
                }
                pwriter.close();
            } catch (IOException e) {
